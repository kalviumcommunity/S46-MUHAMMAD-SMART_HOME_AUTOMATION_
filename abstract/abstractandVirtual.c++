...
public:
    ...
    // Pure virtual function to display the status (making Appliance an abstract class)
...
public:
    ...
    // Override virtual function to display status of the light
    void displayStatus() const override {
        cout << "Light is " << (isOn() ? "ON" : "OFF") << " with brightness level: " << brightness << "%" << endl;
    }
};
...
public:
    ...
    // Override displayStatus function
    void displayStatus() const override {
        cout << "SmartLight is " << (isOn() ? "ON" : "OFF") << ". Scheduled turn-on functionality available." << endl;
    }
};
...
public:
    ...
    // Override virtual function to display status of the thermostat
    void displayStatus() const override {
        cout << "Thermostat is " << (isOn() ? "ON" : "OFF") << " with temperature set to " << temperature << "°C." << endl;
    }
};
...
public:
    ...
    void showStatus() const {
        for (int i = 0; i < 3; ++i) {
            smartLights[i].displayStatus();
        }
        for (int i = 0; i < 2; ++i) {
            thermostats[i].displayStatus();
        }
